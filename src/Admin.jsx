...
function StatCard({ label, value, sub, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 28, fontWeight: 700, color: color || '#222' }}>{fmt(value)}</div>
...
export default function Admin() {
  const [settings, setSettings]     = useState({ zenrows_key: '', scraperapi_key: '' });
  ...
  // Load masked key + status on mount, then poll every 3 s while running
  useEffect(() => {
    fetchSettings();
    fetchStatus();
  }, []);
  ...
  const fetchStatus = () =>
    fetch(`${API}/status`)
      ...
      .then(d => {
        setStatus(d);
        setRunning(d.pipeline?.running || false);
      })
      .catch(() => {});
  ...
  const startPipeline = () => {
    ...
    // Convert YYYY-MM-DD → MM/DD/YYYY for harvester.py
    const toSlash = d => d ? d.split('-').slice(1).concat(d.split('-')[0]).join('/') : '';
    fetch(`${API}/run`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const stopPipeline = () => {
    fetch(`${API}/stop`, { method: 'POST' })
      .then(() => { setRunning(false); fetchStatus(); });
  };
  ...
  return (
    <div style={{ padding: '30px 40px', fontFamily: 'Segoe UI, sans-serif', maxWidth: 960 }}>
      ...
      <p style={{ color: '#666', marginTop: 0, marginBottom: 28, fontSize: 13 }}>
        Spider MJCS for case numbers → scrape full case pages via ZenRows → parse into structured DB tables.
      </p>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 30, flexWrap: 'wrap' }}>
        ...
        <StatCard label="Remaining" value={db.remaining} color={db.remaining > 0 ? '#e74c3c' : '#27ae60'} />
      </div>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '0 0 380px' }}>
          ...
          <div style={card}>
            ...
            <p style={hint}>
              Current key: <code style={{ background: '#f5f5f5', padding: '2px 6px', borderRadius: 4 }}>
          ...
          <div style={{ ...card, marginTop: 20 }}>
            ...
            <p style={hint}>Select the filing-date window to spider + scrape + parse.</p>

            <label style={labelStyle}>Start date <span style={{ color: '#e74c3c' }}>*</span></label>
            <input type="date" value={startDate}
              onChange={e => setStartDate(e.target.value)} style={inputStyle} />

            <label style={{ ...labelStyle, marginTop: 12 }}>End date <span style={{ color: '#888' }}>(optional, defaults to today)</span></label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              {!running ? (
                <button onClick={startPipeline} style={{ ...btnPrimary, flex: 1 }}>
                  ▶ Start Pipeline
                </button>
              ) : (
                <button onClick={stopPipeline} style={{ ...btnDanger, flex: 1 }}>
                  ■ Stop
                </button>
              )}
            </div>
            ...
            <p style={{ fontSize: 11, color: '#aaa', marginTop: 10 }}>
              Pipeline runs 3 steps: Spider (discover) → Scraper (download via ZenRows) → Parser (extract data)
            </p>
          </div>
          ...
          <div style={{ ...card, marginTop: 20 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
              {[
                ...
                <button key={days} style={btnSecondary}
                  onClick={() => {
                    const d = new Date();
                    const s = new Date(d - days * 86400000);
        ...
        <div style={{ flex: 1, minWidth: 300 }}>
          ...
          {/* How to cycle ZenRows accounts */}
          <div style={{ ...card, marginTop: 20, background: '#fffbf0', borderLeft: '4px solid #f39c12' }}>
            <h3 style={{ ...cardTitle, color: '#b7770d' }}>Rotating Free Accounts (to scrape 2 years)</h3>
            <ol style={{ paddingLeft: 18, fontSize: 13, color: '#555', lineHeight: 1.8 }}>
              <li>Go to <a href="https://www.zenrows.com" target="_blank" rel="noreferrer">zenrows.com</a> → create free account (1,000 requests)</li>
              <li>Copy your API key → paste it above → Save</li>
              <li>Pick a date range → click <strong>Start Pipeline</strong></li>
              <li>When the key runs out (1,000 cases scraped), create a new ZenRows account</li>
              <li>Paste the new key → repeat from the next date where you left off</li>
            </ol>
            <p style={{ fontSize: 12, color: '#888' }}>
              Each ZenRows account = ~1,000 cases fully scraped. 2 years ≈ 5,000–50,000 foreclosure cases.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#555', marginBottom: 4 }}>
        <span>{label}</span>
        <span>{fmt(value)} / {fmt(total)} ({p.toFixed(1)}%)</span>
      </div>
      <div style={{ background: '#eee', borderRadius: 4, height: 8 }}>
        <div style={{ width: `${p}%`, background: color, height: 8, borderRadius: 4, transition: 'width .4s' }} />
      </div>
    </div>
  );
}
...
const cardTitle = { margin: '0 0 12px', fontSize: 15, fontWeight: 600 };
const hint      = { fontSize: 13, color: '#666', margin: '0 0 10px' };
